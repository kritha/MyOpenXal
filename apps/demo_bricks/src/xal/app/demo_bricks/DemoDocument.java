...
public class DemoDocument extends XalDocument {
    ...
    public void makeMainWindow() {
		URL url = null;
		try {
			url = new File("E:\\","gui.bricks" ).toURI().toURL();
			System.out.println(url);
		} catch (MalformedURLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		final WindowReference windowReference = new WindowReference(url, "JFrame",this);
		//final WindowReference windowReference = getDefaultWindowReference( "JFrame", this );
        mainWindow = (XalWindow)windowReference.getWindow();
		
		final JList<String> magnetList = (JList<String>)windowReference.getView( "List" );
		...
		final JButton starButton = (JButton)windowReference.getView( "StartButton" );
	...
	protected void showHelloDialog() {
		final WindowReference helloDialogRef = getDefaultWindowReference( "JDialog", mainWindow );
		final JDialog dialog = (JDialog)helloDialogRef.getWindow();
		
		final JButton okayButton = (JButton)helloDialogRef.getView( "Button" );
