...
public class HistogramDocument extends AcceleratorDocument implements DataListener, ChannelHistogramListener {
   ...
   private void configureWindow( final WindowReference windowReference ) {
	   ...
	   channelSelectionButton.addActionListener( newChannelAddHandler() );	   
   }
	...
	private ActionListener newChannelAddHandler() { 
		return new ActionListener() {
			public void actionPerformed( final ActionEvent event ) {
				...
				if ( sequence != null ) {
					...
					if(channelRefSelector==null){
						getAcceleratorWindow().displayError( "getInstanceFromNodes Error", "You must check context to find the error source." );
						return;
					}
	...
	private void rm_histlistener() {
		//removeChannelHistogramListener
		MODEL.removeChannelHistogramListener(this);
	}
	
	/** add the specified listener from receiving channel histogram events from this instance */
	private void add_histlistener() {
		//removeChannelHistogramListener
		MODEL.addChannelHistogramListener(this);
	}
