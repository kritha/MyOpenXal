...
//	}
    ...
    //自定义命令，就是在commander中添加菜单、工具栏或者按钮的事件响应程序，返回action对象并添加到commander中
	//问题是各个action和空间是怎么绑定在一起的
    ...
    //注意函数根本没有使用到参数里传递的adaptor，所以makemainwindow中传递的参数是无效的
	//这个函数就是从va文件中读取sequence、noise、offset相关的设置参数
	...
	//利用filed Book channels，来更新达到上下限（warning和alarm）的pv值，新的值为book channel加减不同的offset
    ...
    //获得回读的PV，并写道相应的记录中，这里使用设计值+noise+offset的方法获得readback值
	private void putReadbackPVs() {
		...
		//遍历每个record，利用set PV的值、noise和offset的值生成相应的readback PV的值并写到record当中。
		//利用设计值来生成，所以只是同步设计值。
	...
	private void putReadbackPVsFromPVLogger() {
		...
		// get the "set" PV value, add noise, and then put to the corresponding readback PV.
		//从pvlogger中获得basis value，并叠加noise+offset，从而得到readback pv的值
		for ( final ReadbackSetRecord record : READBACK_SET_RECORDS ) {
			try {
	...
	//利用默认值来初始化filed book pv（filed*polarities）
	 ...
	 */
    //根据设计值生成pv值并发送给channel，这里的过程和configFieldBookPVs一样，调用toCAFromField
...
//			EnvelopeProbeState state = (EnvelopeProbeState) stateIter.next();
		...
		try {
			...
			_diagplot.showsigmazplot(beamp, beamsigmax,beamsigmay,beamsigmaz);
...
//                final PhaseVector coordinates = cmpCalcEngine.computeCoordinatePosition(probeState);
    ...
    //创建readback pv和set pv直接的映射(READBACK_SET_RECORDS)，同时创建各个通道和误差的映射关系
	...
	//当设置完误差参数之后，点击确定调用该函数，把设置的参数保存在下列成员变量当中
	...
	private void syncOnlineModel() {
		if ( vaRunning ) {
			...
			//20160416
			//putReadbackPVs();//从 virtual magnet中更新PV数据
			for ( final ReadbackSetRecord record : READBACK_SET_RECORDS ) {
				try {
					record.updateReadback();
				}
				catch (Exception e) {
					System.err.println( e.getMessage() );
				}
			...
			try {
				...
				//所有的诊断pv都是在运行之后生成的，所以在模拟完之后发布到服务当中
				putDiagPVs();
			}
			catch ( ModelException exception ) {
				System.err.println( exception.getMessage() );
...
class DiagPlot {
	...
	public void showsigmazplot(double[] p,double[] sigmax,double[] sigmay,double[] sigmaz) throws ConnectionException, GetException {
	...
	public void setupPlot(FunctionGraphsJPanel beampositionplot,FunctionGraphsJPanel sigamplot) {
	    ...
	    Datasigmax.setGraphColor(Color.orange);
	    Datasigmay.setGraphColor(Color.green);
