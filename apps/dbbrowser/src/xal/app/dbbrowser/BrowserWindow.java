...
class BrowserWindow extends XalWindow implements SwingConstants {
	...
	protected void requestUserConnection() {
		final ConnectionDictionary dictionary = ConnectionDictionary.getPreferredInstance( "test" );//change from reports to test
		...
		//System.out.println(_model.getDatabaseAdaptor());
		//DatabaseAdaptor.getInstance();
		final Connection connection = dialog.showConnectionDialog( _model.getDatabaseAdaptor() );
		//final Connection connection = dialog.showConnectionDialog();
		if ( connection != null ) {
