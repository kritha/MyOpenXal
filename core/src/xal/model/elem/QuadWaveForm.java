 ...
 *
 */
public class QuadWaveForm extends WaveFormFormula{
...
//	private final double Bf=1.521694559177570;//final B
	public QuadWaveForm(double strength) {
...
//		final double Gi=initParams[3];
//		final double Gf=initParams[4];
//		final double t1=0.8*tr+tc;
//		if (time>=0 && time<tc) {
//			strength=Gi;
//		} else if (time>=tc && time <t1) {
//			strength=Gi+(Gf-Gi)/0.8*Math.pow((time-tc)/tr, 2);
//		} else if(time>=t1 && time<=tt){
//			strength=Gf-(Gf-Gi)/0.2*Math.pow(1-(time-tc)/tr,2);
//		}else {
//			strength=Gf;
//		}
		...
		if (time>=0 && time <t1) {
			strength=Gi+(Gf-Gi)/0.8*Math.pow(time/tr, 2);
		} else if(time>=t1 && time<=tr){
			strength=Gf-(Gf-Gi)/0.2*Math.pow(1-time/tr,2);
		}else {
			strength=Gf;
		}
	}
