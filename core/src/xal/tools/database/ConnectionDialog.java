...
public class ConnectionDialog extends JDialog {
	...
	public void loadDictionary( final ConnectionDictionary dictionary ) {
		...
		//System.out.println(adaptorClass);
		//System.out.println(dictionary.getURLSpec());
...
//		System.out.println(dictionary.getURLSpec());
//		System.out.println(dictionary.getUser());
//		System.out.println(dictionary.getPassword());
//		System.out.println(dictionary.getDatabaseAdaptor());
		try {
//			System.out.println(databaseAdaptor);
	 ...
	 * Attempt to connect to the database using the supplied database adaptor and the connection
	 * dictionary specified by the user via the dialog box.
	 * add in 2016/01/30 by Yang Ye
	 * the upper one transfer a parameter databaseAdaptor of null, because browser model can't
	 * Initialize databaseAdaptor correctly, so user must input adapter in the connection dialog
	 * @return                 the new connection or null if the user canceled the dialog
	 */
	public Connection showConnectionDialog() {
		ConnectionDictionary dictionary = showDialog();
...
//		System.out.println(dictionary.getDatabaseAdaptor());
		try {
			...
			JOptionPane.showMessageDialog( getOwner(), exception.getMessage(), "Connection Error!", JOptionPane.ERROR_MESSAGE );
			Logger.getLogger("global").log( Level.SEVERE, "Database connection error.", exception );
			return showConnectionDialog( (JFrame)getOwner(), dictionary.getDatabaseAdaptor(), _dictionary );
		}
	}
	 ...
	 * implemented by pure java, not a bricks file */
